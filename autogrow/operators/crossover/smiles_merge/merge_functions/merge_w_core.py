    ...
    for r_groups in rs_chosen_smiles:
        ...
        for frag in r_groups:
            ...
            for anchor_atom_iso in list(anchor_to_connection_dict.keys()):
                ...
                # Some fragments can carry anchor isotope labels that are not
                # present in the MCS/core (e.g., after labeling/break steps),
                # which would otherwise raise a KeyError and abort the whole
                # crossover run. Treat this as a failed merge attempt.
                idx_for_anchor = core_merg_iso_to_idx_dict.get(anchor_atom_iso)
                if idx_for_anchor is None:
                    return None
                ...
                if list_of_atom_idx is None or list_of_bond_types is None:
                    return None
...
def unpack_lists_of_atoms_and_bond_type(anchor_to_connection_dict, anchor_atom_iso,
    ...
    if type(connection_list[0]) == int:
        ...
        atom_idx = core_merg_iso_to_idx_dict.get(atom_iso)
        if atom_idx is None:
            return None, None
